...
public class AudioVisualizer : MonoBehaviour
{
	...
	[Range(1, 200)] public float heightMultiplier;
	...
	public int highestBarHit = 0;
	private int curr = 0;
	private float prevInten = 0.0f;
	private float sensThreshold = 1.0f;
	...
	*/
	...
	void Update()
	{
		...
		for (int i = 0; i < audioSpectrumObjects.Length; i++)
		{
			...
			audioSpectrumObjects[curr].GetComponent<MeshRenderer>().material = pink;

			// see if there is a new highest bar hit
			if (intensity > sensThreshold && intensity > prevInten)
			{
				highestBarHit = i;
				//Debug.Log (highestBarHit);
			}
